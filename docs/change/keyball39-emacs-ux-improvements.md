...
ステータス: 優先度1の案A、優先度2、優先度5を実装済み（実機の基本動作確認済み）。
優先度3・4・6と将来候補は検討中
...
Combo対策後のファームウェアは **26,764 / 28,672 bytes** で、
空きは **1,908 bytes**。Auto Mouse廃止直後の **26,502 bytes** から
Mark安定化で **250 bytes**、Combo対策で **12 bytes** 増加した。比較ビルドの詳細は
...
2026-09-14、`keyball39_emacs_combo.hex`（26,764 bytes）を使った
QMK Toolboxでの書き込み・確認手順の案内後、ユーザーから「問題なし！OKです」と
報告を受けた。優先度1・2・5を含む現行ファームウェアの基本動作確認済みとする。

案内した確認項目は、通常入力とAuto Mouse無効化、5種類のクリック、
連続クリック、ドラッグ・ボタン解放、修飾キー併用、NumFn・縦横スクロール、
Mark選択・ABORTである。項目ごとの操作ログ、誤発火率や押下間隔の数値、
長期使用の結果は記録していない。以下の詳細チェックリストは、
今後の変更時の確認用として残す。
...
基本動作は上記の実機確認結果を参照。
次は両側へ同じファームウェアを書き込んだ際の詳細チェックリストであり、
項目ごとの実機結果は記録していない。
...
QMK Mouse Keys経路を使用する。実機のPointing Device経路やアプリ上の選択動作は
自動テストの対象外であり、基本動作の実機確認結果は上記の記録を参照。
...
## 優先度5: Comboの誤発火対策（実装済み）

キー配置と5種類のマウスボタンComboは維持する。
...
QMK 0.22.14の既定50 msから30 msへ短縮し、最初のCombo候補キーから
受付時間を計る。後続の候補キーによるタイマー延長をなくす。
31 ms以上離れた2キーのロールを文字入力に戻し、受付時間内の同時押しは
従来のクリックとして扱う。押下順序は制限せず、短いクリックも長押しも許可する。
ボタンは最後の構成キーを離すまで保持し、ドラッグと修飾キー併用を維持する。

Strict timerはComboごとではなく全体で共有される。
例えば `M`、20 ms後に `J`、さらに20 ms後に `K` と押すと、
`J + K` だけなら間隔が短くても、`M` から始まった受付時間を延長しない。
別のComboを混ぜた速い連続操作は、従来より成立しにくくなる可能性がある。
また、30 ms以内で重なる通常入力は依然クリックになり得る。
30 msは初期調整値であり、誤発火率や押しやすさを実機で測定した値ではない。

### 採用しない設定

- `COMBO_ONLY_FROM_LAYER` は「そのレイヤーでだけ有効」という制限ではない。
  他レイヤーでも指定レイヤーのキー配置を参照するため、Baseを指定すると
  記号・Fn・Emacsの操作がクリックに置き換わり得る。今回は使用しない。
- `combo_should_trigger()` によるBase限定や入力直後の抑止は追加しない。
  `C-x` の透過キーや、文字・修飾キー操作直後の意図したクリックまで
  無効化するような操作変更を避ける。
- タップ専用の `COMBO_MUST_TAP_PER_COMBO` / `get_combo_must_tap()` は
  クリックドラッグを妨げるため使用しない（QMK 0.22.14の設定名）。

各レイヤーの実際のキーコードで判定する仕様を維持する。
`C-x` がBaseへ透過する場合の中クリック・戻る・進むと、
Emacsレイヤーを保持したままの `C-x` 操作も、専用の回帰ケースで扱う。
キー配列、Combo定義、Mark、Key Override、スクロール判定は変更しない。
...
`tests/keyball39_emacs_mark/test_combo.cpp` に48件を追加した。
productionの `keymap.c` に加えて `config.h` も直接読み込むようにし、
テスト側だけ異なるCombo設定で通ることを避ける。
Comboケースは全レイヤーのproduction配列を使い、QMK 0.22.14の
入力処理から送られたキーボード・マウスレポートを検査する。

- 5組すべてで1・29・30 msの押下間隔がクリックになり、
  31・40・50・51 msではクリックせず元の2キーが順番どおりに入力される。
- クリックの押下順と解放順の全組み合わせ、1秒の保持、短い3連続クリック、
  Ctrl・Alt・GUI・Shift併用、Layer Tap中のボタン解放を検証する。
- 最初の候補キーから31 msでクリック／単独キーのレポートが出る時刻と、
  別のCombo候補キーが受付時間を延長しないことを確認する。
- 記号・NumFn・Emacsの各レイヤーと、Base／Emacsへ透過する `C-x` の
  既存動作、ワンショット消費・ボタン解放を確認する。

変更前の設定で境界・タイマー関連8件がすべて失敗し、変更後は同じ8件を含む
75件（既存Mark 27件＋Combo 48件）が成功した。
shuffle seed 914・915・916で3回実行して計225ケースが成功した。
`mise run keymap:check` も成功した。
`qmk lint -kb keyball/keyball39` は変更前後とも既存の `via.json` 指摘で
終了コード1となり、出力は一致した。

同じQMK 0.22.14・固定コンテナ・AVR GCC 8.3.0で比較ビルドし、
Flashは **26,752 → 26,764 bytes（+12 bytes）**、
静的RAMは **1,515 bytesで不変**。Flash空きは **1,908 bytes**。
ビルド条件と再現コマンドは[容量・最適化調査](../research/keyball39-emacs-optimization.md#combo対策後の再計測)に記載した。

RGB・スクロールsetterはスタブ、マウスはテスト用Mouse Keys経路を使う。
Pointing Deviceの実機経路は自動テストの対象外であり、
基本動作は実機確認済み。実際の誤発火率は未測定。
再現コマンドはキーマップREADMEの回帰テスト手順を参照。

### 実機確認

2026-09-14の実機確認では、ユーザーから基本動作に問題なしとの報告を受けた。
今後の設定変更や長期使用では、次の観点で確認を続ける。

- 普段の日本語・英語入力での誤発火と、5種類の意図したクリックの成立しやすさ
- 連続クリック、修飾キー併用、長いドラッグと片方ずつ離した際のボタン解放
- スクロール・Layer Tap・`C-x` 操作との併用

実機測定で値を調整する場合は、回帰テストの30 ms境界も合わせて更新する。
...
2. Auto Mouseを無効化する（実装済み、実機の基本動作確認済み）
3. Mark処理をweak modifier化する（実装・QMK回帰テスト済み、実機の基本動作確認済み）
4. OLEDへMarkと `C-x` の状態を表示する
5. `ONESHOT_TIMEOUT` とABORT処理を追加する
6. Comboの受付時間を短縮しタイマー延長を防ぐ（優先度5として先行実装・QMK回帰テスト済み、実機の基本動作確認済み）
7. 実際に誤判定するLayer Tapだけ調整する
8. 容量に余裕があればRepeat Keyを試す
